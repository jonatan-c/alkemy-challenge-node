  ...
  try {
    ...
    if (movies2 == null || movies2.length == 0 || movies2 == undefined) {
      ...
      fs.writeFileSync(
        path.join(
          __dirname,
          "../images/moviesOrSerie_images/output/" +
            movies2.id_ms +
            "imagen.png"
        ),
        movies2.image_ms
      ...
      const imagesDir = fs.readdirSync(
        path.join(__dirname, "../images/moviesOrSerie_images/output/")
      );
      // get
      const info = {
        information: [
...
async function getMovieQuery(req, res) {
  ...
  result.map((img) => {
    fs.writeFileSync(
      path.join(
        __dirname,
        "../images/moviesOrSerie_images/output/" + img.id_ms + "imagen.png"
      ),
      img.image_ms
    );
  });
...
async function addMovie(req, res) {
  ...
  const data = fs.readFileSync(
    path.join(
      __dirname,
      "../images/moviesOrSerie_images/input/",
      req.file.filename
    )
  );
