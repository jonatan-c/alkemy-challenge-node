  ...
  try {
    ...
    if (movies2 == null || movies2.length == 0 || movies2 == undefined) {
      ...
      fs.writeFileSync(
        path.join(
          __dirname,
          "../images/character_images/output/" +
            movies2.id_character +
            "imagen.png"
        ),
        movies2.image_character
      );
      const imagesDir = fs.readdirSync(
        path.join(__dirname, "../images/character_images/output/")
      );
...
async function getCharacterQuery(req, res) {
  ...
  movies2.map((img) => {
    fs.writeFileSync(
      path.join(
        __dirname,
        "../images/character_images/output/" + img.id_character + "imagen.png"
      ),
      img.image_character
    );
  });
...
async function addCharacter(req, res) {
  ...
  const data = fs.readFileSync(
    path.join(__dirname, "../images/character_images/input/", req.file.filename)
  );
